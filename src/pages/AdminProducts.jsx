  ...
  return (
    <div className='flex w-full h-screen'>
      ...
      <div className='w-full flex flex-col items-center pt-[180px] lg:pt-[50px]'>
        ...
        <div className="flex flex-col justify-center items-center w-full pt-5 pb-10">
              ...
              {(products.length !== 0) ? (
                products?.map((each, index) =>
                  <tr className="w-full text-center" key={index}>
                    <th className="border border-slate-400 lg:w-[25%]">
                      {each.name}
                    </th>
                    <th className="border border-slate-400 lg:w-[20%]">
                      {each.category_id.name}
                    </th>
                    <th className="border border-slate-400 lg:w-[10%]">
                      {each.price}
                    </th>
                    <th className="border border-slate-400 lg:w-[10%]">
                      {each.stock}
                    </th>
                    <th className="border border-slate-400 lg:w-[5%]">
                      {each.sex}
                    </th>
                    <th className="border border-slate-400 lg:w-[7%]">
                      <div className='flex justify-center'>
                        <svg onClick={() => setShow(!show)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6 mr-2 cursor-pointer transition hover:scale-105">
                ...
                <tr>
                  <td colSpan={6} className="w-[200px] p-2">No registered products</td>
                </tr>
              )}
      ...
      {show && <ModalNewProduct show={show} setShow={setShow} setAdd={setAdd} add={add} />}
    </div>
  )
}
